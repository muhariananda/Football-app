...
class EventLastViewModel(private val footballRepository: FootballRepository) : ViewModel() {

    var data: LiveData<List<EventsEntity>>? = null

    fun initLast(leagueId: String) {
        if (data != null) {
            return
        }

        data = footballRepository.getLastEvent(leagueId)
    }
}